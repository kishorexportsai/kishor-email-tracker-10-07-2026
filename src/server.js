...
console.log('[Email] ========================================');
console.log('[Email] Sender Name:', BREVO_SENDER_NAME);
console.log('[Email] Sender Email:', BREVO_SENDER_EMAIL);
console.log('[Email] API Key Set:', BREVO_API_KEY ? '✅ YES' : '❌ NO');
console.log('[Email] ========================================');

// ✅ FIXED: Send email with FULL Brevo response logging
  ...
  return new Promise((resolve) => {
    try {
      ...
      if (!BREVO_SENDER_EMAIL) {
        console.error('[Email] ❌ BREVO_SENDER_EMAIL is NOT set');
        resolve(false);
        return;
      }
      ...
      const emailData = JSON.stringify({
        ...
        sender: { 
          ...
          email: BREVO_SENDER_EMAIL  // ✅ FIXED: Using verified sender!
        }
      });
      ...
      const req = https.request(options, (res) => {
        ...
        res.on('end', () => {
          // ✅ DETAILED LOGGING: Show EVERY response from Brevo
          console.log('[Email] ==============================');
          console.log('[Email] BREVO API RESPONSE');
          console.log('[Email] ==============================');
          console.log('[Email] Status Code:', res.statusCode);
          console.log('[Email] Headers:', JSON.stringify(res.headers, null, 2));
          console.log('[Email] Response Body:', data);
          console.log('[Email] ==============================');
          ...
          if (res.statusCode === 201) {
            ...
            console.error('[Email] ❌ 400 Bad Request - Email format issue');
            try {
              const errorData = JSON.parse(data);
              console.error('[Email] Brevo Error:', errorData.message);
            } catch (e) {
              console.error('[Email] Response:', data);
            }
            resolve(false);
          } else if (res.statusCode === 401) {
            console.error('[Email] ❌ 401 Unauthorized - API key is INVALID');
            resolve(false);
          } else if (res.statusCode === 403) {
            console.error('[Email] ❌ 403 Forbidden - Sender not verified or blocked');
            resolve(false);
          } else if (res.statusCode === 429) {
            console.error('[Email] ❌ 429 Rate Limited');
            resolve(false);
          } else {
            console.error('[Email] ❌ Error Status:', res.statusCode);
            resolve(false);
          }
        });
      });
...
async function sendReminderToUser() {
  try {
    ...
    if (error) {
      console.error('[UserReminder] Failed to load unreplied emails:', error.message);
      return false;
    }
...
async function checkUserReplyAndAlertManager() {
  try {
    ...
    if (error) {
      console.error('[ManagerAlert] Failed to load unreplied emails:', error.message);
      return false;
    }
...
// ✅ NEW: Comprehensive email diagnostics endpoint
app.get('/api/test/email-diagnostics', async (req, res) => {
  ...
  diagnostics.checks.sender_email = {
    ...
    description: BREVO_SENDER_EMAIL ? `Sender configured as: ${BREVO_SENDER_EMAIL}` : 'BREVO_SENDER_EMAIL is MISSING'
  };
...
// ✅ NEW: Direct test email with detailed logging
